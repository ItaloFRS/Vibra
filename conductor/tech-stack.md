...
A arquitetura e as tecnologias selecionadas para o projeto VIBRA focam em consistência transacional (ingressos) e alta interatividade (match/chat), além da fidelidade ao Google Stitch.
...
•	**Gerenciamento de Formulários:** React Hook Form + Zod para validação estrita de dados e feedback imediato.
...
- O coração da experiênca "social" e "transacional" do Vibra.

- **Stack Técnica:**
    - Framework: React Native + Expo (SDK 54+, Managed Workflow)
    - Localização: Expo Location (Geocodificação Reversa)
    - Navegação: Expo Router (File-based)
    - Estilização: NativeWind (Tailwind CSS) com Suporte a Dark Mode dinâmico.
    - Estado: TanStack Query + ThemeContext global.
    - Storage: Expo SecureStore (Persistência de Sessão e Preferências de Tema)
    - Ícones: Lucide React Native + React Native SVG
    - Sensores: Expo Sensors (Acelerômetro para Match por gestos)
    - UI/Visual: Expo Blur (Glassmorphism estável para Android/iOS)
...
### 4. *Web Home (B2C Landing Page)*
- Focada em SEO, performance e conversão visual premium.

•	**Framework:** Next.js + React (App Router).
•	**Animações:** Framer Motion (UI/Scroll), Remotion (Scroll-driven video/3D logic) e Lenis Scroll para scroll suave global.
•	**Estilização:** Tailwind CSS (configuração compartilhada).
•	**Assets:** Vídeos em background otimizados e imagens servidas via Cloudinary.


### 5. Fluxo de Sincronia Design-Código
...
### 7. Estrutura de Pastas (dentro de /apps)
...
│   │   ├── components/     ## UI Kit, Sections (Hero, Wallet)
│   │   ├── lib/            ## Utils (cn)
│   │   └── hooks/          ## useScroll
...
- •	**Processamento de Documentos:** Apache PDFBox 3.x (PDF para Imagem/Texto) e ZXing (Detecção de QR Code).
- •	**Inteligência Artificial:** Google Gemini 2.0 Flash (NLP/OCR) para extração inteligente de dados.
- •	**Comunicação Real-time:** Spring WebSocket com protocolo STOMP.
- •	**Segurança:** Spring Security + Stateless JWT + Firebase Admin SDK para validação de tokens Google.
•	**Comunicação:** JavaMailSender (Gmail SMTP) para envio de códigos de verificação OTP.
...
- •	**Destaque:** Validação rigorosa de CPF, verificação de e-mail por código (OTP) e armazenamento de preferências enriquecidas (Vibes, Match) em JSONB.
...
- Para garantir performance máxima no Mobile e fidelidade visual aos protótipos do Stitch, o projeto utiliza uma arquitetura de Object Storage with processamento dinâmico.
...
## Localização do MAVEN : **"C:\Users\italo\Maven\apache-maven-3.9.14"**
